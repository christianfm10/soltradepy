...
class MoralisClient:
    ...
    async def get_graduated_tokens_by_exchange_pumpfun(
        self, limit: int = 10, cursor: str = None
    ) -> moralis_models.MoralisGraduatedTokensResponse:
        endpoint = "/token/mainnet/exchange/pumpfun/graduated"

        params = {"limit": limit, **({"cursor": cursor} if cursor else {})}
        # if cursor:
        #     params["cursor"] = cursor
        return moralis_models.MoralisGraduatedTokensResponse(
            **await self._moralis_request("GET", endpoint, params)
        )
